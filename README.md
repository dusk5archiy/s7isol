...
- Run `bash bin/init.sh` to create `.env` file in the project root folder.
- Modify `S7ISOL_ROOT` to the folder that will contain the new profile.
...
```bash
export OLD_VSCODE_USER_DATA_DIR="$XDG_CONFIG_HOME/Code"
export OLD_VSCODE_EXTENSIONS_DIR="$HOME/.vscode/extensions"
```
